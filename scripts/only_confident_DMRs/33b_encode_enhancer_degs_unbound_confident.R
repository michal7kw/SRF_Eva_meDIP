#!/usr/bin/env Rscript
#
# ENCODE ENHANCERS OF DEGs DOWN - UNBOUND VERSION (33b)
# =============================================================================
#
...
#
# This version outputs to the 33b directory for the unbound-focused analysis.
#
# =============================================================================
...
cat("==========================================================\n")
cat("ENCODE ENHANCERS OF DEGs DOWN - UNBOUND VERSION (33b)\n")
cat("==========================================================\n")
...
cat(sprintf("      - with DMR: %d (RELIABLE - PRIMARY FOCUS)\n", length(unbound_dmr_idx)))
cat(sprintf("      - no DMR: %d\n", length(unbound_nodmr_idx)))

# =============================================================================
# PHASE 7: CREATE CONTROL SET (MATCHED SIZE FOR UNBOUND WITH DMR)
# =============================================================================

cat("\n=== PHASE 7: Creating Control Set ===\n")

# Match to unbound DMR count for this version
n_target <- length(unbound_dmr_idx)
set.seed(42)
...
if (nrow(enhancers_control_pool) > n_target && n_target > 0) {
    ...
    cat(sprintf("  Subsampled Control enhancers (matched to Unbound+DMR): %d\n", nrow(enhancers_control)))
} else {
...
summary_df <- data.frame(
    ...
    Reliability = c("HIGH", "LOW (GFP dropout possible)", "MIXED",
                    "MEDIUM", "HIGH - PRIMARY FOCUS", "LOW (GFP dropout possible)", "MATCHED")
)
...
cat("\n")
cat("==========================================================\n")
cat("PREPARATION COMPLETE - UNBOUND VERSION (33b)\n")
cat("==========================================================\n")

cat("\nKey insight:\n")
cat("  This version focuses on UNBOUND enhancers with DMR overlap\n")
cat("  which provides more regions (155) for reliable analysis.\n\n")
cat("  TES-bound enhancers with DMR overlap are very few (~1),\n")
