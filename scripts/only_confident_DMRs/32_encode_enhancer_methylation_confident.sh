#!/bin/bash
#SBATCH --job-name=32_encode_enh_conf
#SBATCH --account=kubacki.michal
#SBATCH --partition=workq
#SBATCH --time=4:00:00
#SBATCH --mem=32G
#SBATCH --cpus-per-task=16
#SBATCH --output=logs/32_encode_enhancer_methylation_confident.out
#SBATCH --error=logs/32_encode_enhancer_methylation_confident.err

# =============================================================================
# ENCODE ENHANCER METHYLATION - HIGH-CONFIDENCE VERSION (TES-BOUND FOCUS)
# =============================================================================
#
# Purpose: Analyze methylation at ENCODE-defined enhancers stratified by:
...
#       See 32b version for Unbound-focused analysis with more regions.
#
# =============================================================================
...
OUTDIR="output/32_encode_enhancer_confident"
mkdir -p ${OUTDIR}
mkdir -p logs
...
Rscript 32_encode_enhancer_methylation_confident.R
...
if [ "$N_TES_DMR" -ge $MIN_REGIONS ] && [ "$N_UNBOUND_DMR" -ge $MIN_REGIONS ]; then
    ...
    # Compute matrix with DMR-stratified groups
    computeMatrix reference-point \
        --referencePoint center \
        -S $TES_METH $GFP_METH $TES_BIND $TEAD1_BIND \
        -R ${OUTDIR}/TES_bound_enhancers_with_DMR.bed \
           ${OUTDIR}/TES_bound_enhancers_no_DMR.bed \
    ...
    plotProfile -m ${OUTDIR}/encode_enhancer_dmr_matrix.gz \
        ...
        --plotTitle "ENCODE Enhancers: Methylation (DMR-stratified)" \
        --plotHeight 14 \
        --plotWidth 18 \
    ...
    echo ""
    echo "NOTE: Use 32b_encode_enhancer_unbound_confident.sh for Unbound-focused analysis"
fi
