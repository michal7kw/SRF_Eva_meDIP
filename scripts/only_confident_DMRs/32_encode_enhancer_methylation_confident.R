#!/usr/bin/env Rscript
#
# ENCODE ENHANCER METHYLATION - HIGH-CONFIDENCE VERSION
# =============================================================================
#
...
#
# Key insight: Original analysis showed 91% hypermethylated DMRs, but this is
# an artifact of GFP library dropout. High-confidence DMRs show 65% hypomethylation.
#
# =============================================================================
...
cat("==========================================================\n")
cat("ENCODE ENHANCER PREPARATION - HIGH-CONFIDENCE VERSION\n")
cat("==========================================================\n")
...
# Output directory
OUTPUT_DIR <- "output/32_encode_enhancer_confident"
...
# Subsample unbound for comparison (match TES-bound with DMR count)
set.seed(42)
n_sample <- min(length(tes_bound_dmr), length(unbound_dmr))
if (length(unbound_dmr) > n_sample) {
    unbound_dmr_subsample <- sample(unbound_dmr, n_sample)
} else {
    unbound_dmr_subsample <- unbound_dmr
}
write_bed(encode_gr[unbound_dmr_subsample], file.path(OUTPUT_DIR, "Unbound_enhancers_DMR_subsampled.bed"))
...
cat("\nKey insight:\n")
cat("  Enhancers with high-conf DMR overlap have RELIABLE methylation\n")
cat("  because BOTH TES and GFP samples have >2 reads.\n\n")
cat("  Original analysis: 91% hypermethylated DMRs (ARTIFACT)\n")
cat("  High-confidence: 35% hyper / 65% hypo (REAL BIOLOGY)\n")
